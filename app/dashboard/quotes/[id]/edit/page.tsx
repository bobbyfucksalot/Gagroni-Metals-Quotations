...
export default function EditQuotePage() {
  ...
  // Load target quote and master data
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoadingQuote(true);
        const [resQuote, resClients, resProducts, resSettings, resCurrency] = await Promise.all([
        ...
        if (!resQuote.quote) {
          showToast('Quotation not found');
          setLoadingQuote(false);
          return;
        }

        const q: Quote = resQuote.quote;
        setOriginalQuote(q);

        // Pre-fill states from target quotation
        setQuoteNumber(q.quoteNumber || '');
        setStatus(q.status || 'Draft');
        setTitle(q.title || 'Fabrication & Structural Metal Works');
        ...
        setTaxMode(q.taxMode || 'gst_intra');
        setTheme(q.theme || 'tally');

        if (q.currency?.export) {
          setSelectedCurrency(q.currency.export);
        }
        ...
        // Extra dispatch fields
        if (q.corporateFields?.customFields && Array.isArray(q.corporateFields.customFields) && q.corporateFields.customFields.length > 0) {
          setExtraFields(q.corporateFields.customFields);
        } else if (q.corporateFields?.extraFields && Array.isArray(q.corporateFields.extraFields) && q.corporateFields.extraFields.length > 0) {
          setExtraFields(q.corporateFields.extraFields);
        } else if (q.corporateFields) {
          const fields: ExtraFieldItem[] = [
            { id: 'f-1', label: 'Delivery Note', value: q.corporateFields.deliveryNote || '' },
            { id: 'f-2', label: "Supplier's Ref.", value: q.corporateFields.supplierRef || '' },
            { id: 'f-3', label: 'Other Reference(s)', value: q.corporateFields.otherReferences || '' },
            { id: 'f-4', label: "Buyer's Order No.", value: q.corporateFields.buyerOrderNo || '' },
            { id: 'f-5', label: 'Dispatched Through', value: q.corporateFields.dispatchedThrough || '' },
            { id: 'f-6', label: 'Destination', value: q.corporateFields.destination || '' },
            { id: 'f-7', label: 'Terms of Delivery', value: q.corporateFields.termsOfDelivery || '' },
            { id: 'f-8', label: 'Payment Terms', value: q.corporateFields.paymentTerms || '' },
          ];
          setExtraFields(fields);
        }

        if (q.documentModules) {
          setDocumentModules({
            dualSignOff: q.documentModules.dualSignOff ?? false,
            amountInWords: q.documentModules.amountInWords ?? true,
            hsnCodes: q.documentModules.hsnCodes ?? true,
            thumbnails: q.documentModules.thumbnails ?? true,
          });
        }

        if (q.marketingPage) {
          setMarketingPage({
            enabled: q.marketingPage.enabled ?? false,
            imageUrl: q.marketingPage.imageUrl || '',
            title: q.marketingPage.title || 'Product Showcase & Engineering Portfolio',
            description: q.marketingPage.description || 'Precision Engineered Metal & Stainless Steel Solutions • Heavy-Duty Fabrication',
          });
        }

        if (q.terms) {
          const parsed = q.terms.split('\n').filter(Boolean);
          if (parsed.length > 0) setTermsClauses(parsed);
        }
        ...
        if (q.totals?.extraDiscountPercent !== undefined) {
          setExtraDiscountPercent(q.totals.extraDiscountPercent);
        }
        if (q.totals?.shipping !== undefined) {
          setShipping(q.totals.shipping);
        }

        if (q.signature?.imageUrl) {
          setSignatureData(q.signature.imageUrl);
        } else if (resSettings?.settings?.signatureUrl) {
        ...
        showToast('Failed to load quotation for editing');
      } finally {
        setLoadingQuote(false);
      }
    };
  ...
  const currentRateAgainstINR =
    exchangeRates[selectedCurrency]?.rateAgainstINR ||
    1 / (SUPPORTED_CURRENCIES[selectedCurrency]?.defaultInrPerUnit || 1);

  const currentInrPerUnit =
  ...
  const previewQuote: Quote = {
    id: originalQuote?.id || quoteId,
    quoteNumber: quoteNumber || originalQuote?.quoteNumber || 'GM-2026-0046',
    title: title || 'Fabrication & Structural Metal Works',
    ...
    consigneeStateCode,
    status,
    statusHistory: originalQuote?.statusHistory || [],
    issueDate,
    validUntil,
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      showToast('Network error while updating quotation');
      setSubmitting(false);
    }
  };
        ...
        <div className="qc-content">
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <Link href={`/dashboard/quotes/${quoteId}`} className="btn-secondary" style={{ padding: '0 10px', height: '36px' }}>
                <ArrowLeft size={16} />
              </Link>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <span style={{ fontSize: '14px', fontWeight: '800', color: 'var(--accent-emerald)', background: '#ECFDF5', padding: '2px 8px', borderRadius: '6px', border: '1px solid #A7F3D0' }}>
                    {quoteNumber || quoteId}
                  </span>
                  {originalQuote?.version && originalQuote.version > 1 && (
                    <span style={{ fontSize: '11px', color: '#6B7280', background: '#F3F4F6', padding: '2px 6px', borderRadius: '4px' }}>
                      v{originalQuote.version}
                    </span>
                  )}
                </div>
                <div style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>
                  Update line items, client details, GST tax configuration, and corporate terms.
                </div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              {/* Status Selector */}
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', background: '#FAFAF9', padding: '5px 12px', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                <span>Status:</span>
                <select
                  value={status}
                  onChange={(e) => setStatus(e.target.value as QuoteStatus)}
                  style={{ border: 'none', background: 'transparent', fontWeight: '800', color: 'var(--text-primary)', cursor: 'pointer', outline: 'none' }}
                >
              ...
              >
                <Eye size={15} /> {previewMode ? 'Back to Editor' : 'Live Document Preview'}
              </button>
              ...
              >
                <Save size={15} /> {submitting ? 'Saving Changes...' : 'Save Changes'}
              </button>
            </div>
          </div>
          ...
          {previewMode ? (
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
              {/* Quotation Metadata Card: Title, Issue Date, Valid Until */}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: '16px' }}>
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. Fabrication & Structural Metal Works"
                    />
                  </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                        <div>
                          ...
                          <input type="text" className="qc-input" value={clientContactPerson} onChange={(e) => setClientContactPerson(e.target.value)} />
                        </div>
                        <div>
                          <label style={{ display: 'block', fontSize: '11px', fontWeight: '600', marginBottom: '4px' }}>Email Address</label>
                          <input type="email" className="qc-input" value={clientEmail} onChange={(e) => setClientEmail(e.target.value)} />
                        </div>
                      </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '18px', flexWrap: 'wrap', gap: '8px' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '11px', color: 'var(--text-secondary)' }}>
                      Add or edit custom commercial fields (e.g. Delivery Note, Buyer Order No, Dispatched Through, Destination)
                    </div>
                  </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '24px' }}>
                ...
                <div className="qc-card" style={{ padding: '24px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '13px' }}>
                    ...
                    {/* Extra Discount & Shipping Adjustments */}
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', padding: '6px 0', borderTop: '1px dashed var(--border-subtle)', borderBottom: '1px dashed var(--border-subtle)' }}>
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => setExtraDiscountPercent(Number(e.target.value) || 0)}
                        />
                      </div>
                      <div>
                        <label style={{ display: 'block', fontSize: '11px', color: 'var(--text-secondary)', marginBottom: '2px' }}>
                    ...
                    <button
                      ...
                      disabled={submitting}
                      className="btn-primary"
                      style={{ width: '100%', height: '44px', marginTop: '12px', fontSize: '14px', background: 'var(--accent-emerald)', borderColor: 'var(--accent-emerald)' }}
                    >
                      <Save size={16} /> {submitting ? 'Updating Quotation...' : 'Save & Update Quotation'}
                    </button>
                  </div>
                </div>
              </div>
